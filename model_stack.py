...
X=[]
X1=pad_sequences(X1,dtype=str,maxlen=2000,padding="post",value=1)
...
f=pd.read_csv('stack_data.csv',header=None)
y=list(f[4])
y=y[1:]
...
model = Sequential()
model.add(Embedding(len(all_words)+1,300,weights=[embedding_matrix],input_length=2000,trainable=False))
model.add(CuDNNLSTM(2000)) # return sequence=false #relu/leakyrelu
model.add(Dense(1)) #time distributed
